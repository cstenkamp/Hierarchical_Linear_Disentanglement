...
class Primary_Sub_Features():
    ...
    def __init__(self,ent,fea,orderd_features,mds,train_labels,
                 orderd_features_directions, orderd_features_Kappa,primary_sub_Path):
    ...
    def findingPrimaryFeatures(self):
        ...
        """
        self.__primarfeatures, self.__primaryFDirections, self.__primaryFPosIns, self.__prim_PositveIns_vectors,self.__prim_PositveIns_Labels, self.__Prim_feature_Index\
        , self.__clusterprediction = self.__clusAff(self.__orderd_features_directions[:5000],self.__orderd_features[:5000])
    ...
    def LearningFeaturesDirecInPr(self):
        ...
        for pf in range(len(self.__primarfeatures)):
            ...
            if(len(self.__Prim_feature_Index[pf])!=0):
                    ...
                    for fe in  self.__Prim_feature_Index[pf]:
                        ...
                        kappa_scoretest,kappa_score1, f1, direction, acc, TP, FP, TN, FN, predicted,probabilities =runLR1(self.__prim_PositveIns_vectors[pf],np.array(self.__prim_PositveIns_Labels[pf])[:,fe])
                    ...
                    for js in orderd_accuracy_index_:
                        orderd_features_.append(self.__fea[self.__Prim_feature_Index[pf][int(js)]])
                    ...
                    for f in range(0,len(orderd_features_)):
                        ...
                        if kappa_Score_all_test_[orderd_accuracy_index_[f]]>=0.1:
                            print(orderd_features_[f],kappa_Score_all_test_[orderd_accuracy_index_[f]], '****')
                            
                            tempsubFe.append(orderd_features_[f])
                            directionsLGR_TempFiltered.append(orderd_features_directions_[f])
        ...
        print('step11 learning the rest of the featuer directions in  the primary features part of the space is finished')
        ...
        for i in range(0,len(sub_feat)):
            ...
            print()
            if len(sub_feat[i])>=1:
                clus = AffinityPropagation().fit(np.array(sub_feat_directionsfiltered[i]))
                    ...
                    for j in range(0,len(sub_feat[i])):
                        if clus.labels_[j]==n:
                            ...
                            if len(allKappaTest[i])!=0:
                                print(sub_feat[i][j],allKappaTest[i][cc[indexx]],'***')
    ...
    def learning_sub_direction(self):
        ...
        for c in range(0,len(cluster2stLeve_Labeles_all)):
            ...
            for i in cluster2stLeve_Labeles_all[c]:
                ...
                kappa_scoretest,kappa_score1, f1, direction, acc, TP, FP, TN, FN, predicted,probabilities =runLR1(self.__prim_PositveIns_vectors[c],np.array(tempI))
                ...
                num=num+1
                del kappa_score1, f1, direction, acc, TP, FP, TN, FN, predicted,probabilities
                gc.collect()
        ...
        print('step14 learning the directions of the subfeatuers (in the postive part of the space)')        
        ...
        for i in range(0,len(self.__primaryFDirections)):
            ...
            for j in range(0,len(Sub_cluster_directions_ALL[i])):
                ...
                orthvect=Sub_cluster_directions_ALL[i][j]-np.array(((np.dot(Sub_cluster_directions_ALL[i][j],self.__primaryFDirections[i]))/
                                                           (np.dot(self.__primaryFDirections[i],self.__primaryFDirections[i]))))*self.__primaryFDirections[i]
    ...
    def __clusAff(self,directions,terms):
        ...
        cluster_directions,positiveMovies_cluster,PositveIns_vectors\
        ,PositveIns_Labels,clusterPrediction=self.__findingclusterDirections(PrimaryfeatClusters,
                                                                                   terms,directions,
        ...
        return PrimaryfeatClusters, cluster_directions,positiveMovies_cluster,PositveIns_vectors,PositveIns_Labels,features_inPrimary_index,clusterPrediction


    def __findingclusterDirections(self,clusters,terms,terms_Direc,features_index_forechCluster):
        ...
        PrimaryFeatures_labels=self.__findingclusterLabels(clusters,features_index_forechCluster)
        cluster_directions=[]
        cluster_predictions=[]
        cluster_kappa=[]
        cluster_kappa_test=[]
        for i in PrimaryFeatures_labels:
            ...
            kappa_scoretest,kappa_score1, f1, direction, acc, TP, FP, TN, FN, predicted,probabilities = runLR1(self.__mds, i)
        ...
        positiveMovies_cluster,PositveInstances_vectors,PositveInstances_Labels =  self.__findingPositiveInstances(clusters,cluster_predictions)
        return cluster_directions,positiveMovies_cluster,PositveInstances_vectors,PositveInstances_Labels,cluster_predictions
    ...
    def __findingPositiveInstances(self,clustersArray,clustersPrediction):
        ...
        for movie_i in range(0,len (clustersArray)):
            ...
            #print((flat_list))
            PosIn = [x1 for x1 in range(0,len(self.__mds)) if clustersPrediction[movie_i][x1]==1 ]
...
def runLR1(vectors, classes):
    ...
    clf.fit(vectors, classes)#,sample_weight=squared_sample)  # All of the vectors and classes. No need for training data.
...
def finding_directions(fea,space,facet,featuresTerms,GloveVectors,train_labels,Path):
    ...
    for i in  range(0,len(featuresTerms)):
        ...
        kappa_scoretest,kappa_score1, f1, direction, acc, TP, FP, TN, FN, predicted,probabilities = runLR1(space,train_labels[:,fea.index(featuresTerms[i])])
